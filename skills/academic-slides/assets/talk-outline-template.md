# Academic Talk Outline

> Portable Codex-Hermes-Claude replacement authored by Rylai.

- Audience:
- Venue:
- Duration:
- Target slide count:
- Main claim:
- One-sentence audience takeaway:
- Evidence required to support the claim:
- Material reserved for appendix:

## Narrative

| Section | Audience question | Action-title sequence | Evidence | Minutes |
|---|---|---|---|---:|
| Opening | Why should I care? |  |  |  |
| Gap | What is unresolved? |  |  |  |
| Approach | What did you do? |  |  |  |
| Results | What did you learn? |  |  |  |
| Limits | Where does it not apply? |  |  |  |
| Conclusion | What should I remember? |  |  |  |

## Q&A Preparation

- Most likely clarification:
- Strongest methodological challenge:
- Strongest validity challenge:
- Backup evidence needed:
